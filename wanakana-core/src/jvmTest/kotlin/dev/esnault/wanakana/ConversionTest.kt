...
import dev.esnault.wanakana.core.toHiragana
import dev.esnault.wanakana.core.toKana
import dev.esnault.wanakana.core.toKatakana
import dev.esnault.wanakana.helpers.*
import dev.esnault.wanakana.core.utils.safeUpperCase
import org.junit.jupiter.api.DisplayName
import org.junit.jupiter.api.Nested
import org.junit.jupiter.api.TestFactory
import dev.esnault.wanakana.helpers.ROMA_TO_HIRA_KATA
            ...
            HIRA_KATA_TO_ROMA.forEach { (hiragana, katakana, romaji) ->
                if (hiragana.isNotEmpty()) {
                    testEquals(name = "$hiragana -> $romaji", expected = romaji) {
                        dev.esnault.wanakana.core.toRomaji(hiragana)
                    }
                }
                if (katakana.isNotEmpty()) {
                    testEquals(name = "$katakana -> $romaji", expected = romaji) {
                        dev.esnault.wanakana.core.toRomaji(katakana)
                    }
                }
            }
        }
